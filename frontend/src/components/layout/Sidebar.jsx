...
const navigation = [
  ...
  { 
    ...
    icon: UserGroupIcon,
    children: [
      { name: 'Mon lien', href: '/referral' },
      { name: 'Mes filleuls', href: '/referral' },
      { name: 'Commissions', href: '/referral' },
    ]
  },
  { 
    name: 'Documents', 
    ...
    children: [
      { name: 'Factures', href: '/profile' },
      { name: 'Contrats', href: '/profile' },
    ]
  },
  { 
    name: 'Aide', 
...
// Composant pour l'icône de chevron (déplacé plus bas dans le composant)
...
const Sidebar = ({ isMobileOpen, onMobileToggle, isCollapsed, onToggleCollapse }) => {
  ...
  // Mise à jour de l'état mobile lors du redimensionnement
  useEffect(() => {
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    // Ne pas inclure onMobileToggle dans les dépendances pour éviter les boucles infinies
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.pathname, isMobile, isMobileOpen]);
  ...
  return (
    <div className="flex h-full">
      {/* Bouton de réduction/extension du menu - Seulement visible en mode desktop */}
      {!isMobile && (
        <button
          onClick={(e) => {
            e.stopPropagation();
      ...
      <div 
        ...
        style={{
          ...
          pointerEvents: 'auto' // S'assurer que la sidebar peut recevoir des événements
        }}
        onClick={handleSidebarClick}
        onTouchStart={handleSidebarClick} // Pour les appareils tactiles
      >
        {/* Logo et Titre */}
